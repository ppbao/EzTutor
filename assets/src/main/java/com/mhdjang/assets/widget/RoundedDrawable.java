...
public class RoundedDrawable extends Drawable {
    ...
    private final float mInnerShadowWidth = 2;
    ...
    private void updateShaderMatrix() {
        ...
        mInnerShadowRect.set(mDrawableRect);
        mInnerShadowRect.inset(mInnerShadowWidth / 2, mInnerShadowWidth / 2);
        mBitmapShader.setLocalMatrix(mShaderMatrix);
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (mOval) {
            if (mBorderWidth > 0) {
                ...
                canvas.drawOval(mInnerShadowRect, mInnerShadowPaint);
            } else {
                canvas.drawOval(mDrawableRect, mBitmapPaint);
                canvas.drawOval(mInnerShadowRect, mInnerShadowPaint);
            }
            ...
            if (mBorderWidth > 0) {
                ...
                int length = mCornerDirection.size();
                float[] outerRadii = new float[length * 2];

                for (int i = 0; i < length; i++) {
                    if (mCornerDirection.get(i)) {
                        outerRadii[2 * i] = mCornerRadius;
                        outerRadii[2 * i + 1] = mCornerRadius;
                    }
                }

                RoundRectShape roundRectShape = new RoundRectShape(outerRadii, null, null);
                roundRectShape.resize(mDrawableRect.width(), mDrawableRect.height());
                roundRectShape.draw(canvas, mBitmapPaint);
    ...
    public RoundedDrawable setCornerDirection(List<Boolean> direction) {
